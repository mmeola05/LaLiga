...
import com.liga.controller.AlineacionController;
import com.liga.controller.EquipoController;
import com.liga.controller.SimuladorJornada;
...
public class MenuPrincipal {
    ...
    private void registrarUsuario() {
        ...
        List<String> ocupados = leagueRepository.listarUsuarios().stream()
                .map(Usuario::getEquipo)
                .filter(Objects::nonNull)
                .toList();

        List<Equipo> disponibles = leagueRepository.listarEquipos()
                .stream()
        ...
        List<Jugador> jugadores = leagueRepository.buscarJugadorPorEquipo(equipoId);
    ...
    private void menuUsuarioLogueado(Usuario usuario) {
        ...
        do {
            System.out.println("\n=== MI PERFIL (Saldo: " + formatearDinero(usuario.getSaldo()) + ") ===");
            ...
            System.out.println("7. Buscar Jugador");
    ...
    private void buscarJugadores() {
        ...
        List<Jugador> todos = leagueRepository.listarJugadores();
        ...
        if (criterio.equals("N")) {
            ...
            resultados = todos.stream()
                    ...
                    .toList();
        } else if (criterio.equals("P")) {
            System.out.print("Introduce posición (PORTERO, DEFENSA, MEDIO, DELANTERO): ");
            String posStr = sc.nextLine().trim().toUpperCase();
            try {
                Posicion pos = Posicion.valueOf(posStr);
                resultados = todos.stream()
        ...
        for (JugadorMercado jm : mercado) {
            leagueRepository.buscarJugadorPorId(jm.getJugadorId())
                    .ifPresent(j -> System.out.printf(
                            "- ID Mercado: %s | %s (%s) | Precio: %s | Vendedor: %s%n",
                            jm.getId(),
                            j.getNombre(),
                            j.getPosicion(),
                            formatearDinero(jm.getPrecioSalida()),
                            jm.getVendedor()));
        }
    }
    ...
    private void ponerJugadorEnVenta(Usuario usuario) {
        ...
        if (ok) {
            ...
            System.out.println("  - El jugador esté en tu plantilla.");
            System.out.println("  - El jugador no esté ya en venta.");
        }
    }
    ...
    private void comprarJugadorMercado(Usuario usuario) {
        ...
        if (ok) {
            ...
            System.out.println("  - Tengas saldo suficiente.");
            System.out.println("  - No tengas ya 25 jugadores.");
            System.out.println("  - No intentes comprar tu propio jugador.");
        }
    }
